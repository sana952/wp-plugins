...
class PostMetricsPlugin {
    ...
    //callback to create settings menu in admin sidebar
    function pmp_menu(){
      add_menu_page('Post Metrics PLugin', 'Post Metrics', 'manage_options', 
                        'post-metrics', array($this, 'render_menu_page'),
                        'dashicons-chart-line', 200);
      add_submenu_page('post-metrics', 'Post Metrics Settings', 'General Settings', 
                        'manage_options', 'post-metrics', array($this, 'render_menu_page')); 
      $functionalityPageHook = add_submenu_page('post-metrics', 'Post Metrics Functionality', 'Functionality', 
                        'manage_options', 'post-metrics-functionality', array($this, 'render_functionality_submenu_page')); 
      add_action("load-{$functionalityPageHook}", array($this, 'functionality_page_assets'));
    }
    ...
    //function to display HTML on the settings page
    function render_functionality_submenu_page(){ ?>
      Helo from the submenu page
    <?php
    }

    //function to load custom CSS for teh Functionality page
    function functionality_page_assets(){
      wp_enqueue_style('postMetricCss', plugin_dir_url(__FILE__).'style.css');
    }
